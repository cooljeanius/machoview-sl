...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex
{
  ...
  if (selectedNode.details == nil) {
    NSUInteger offset = (selectedNode.dataRange.location + rowIndex * 16U);
    ...
    if (colIndex == OFFSET_COLUMN) {
      ...
      if ([document isRVA] == YES) {
        ...
        return [layout performSelector:@selector(convertToRVA:) /* not sure if this is supposed to be the one from MVLayout or MachOLayout? */
    ...
    if (colIndex == DATA_LO_COLUMN) {
      NSUInteger index = ((len > 8U) ? 8U : len);
    ...
    {
      NSUInteger index = ((len > 8U) ? (len - 8U) : 0U);
  ...
  if (row != nil)
  {
    ...
    if ((colIndex == OFFSET_COLUMN) && ([cellContent length] > 0)) {
      if ([document isRVA] == YES) {
        id layout = [selectedNode.userInfo objectForKey:MVLayoutUserInfoKey];
        cellContent = [layout performSelector:@selector(convertToRVA:) /* as with previous, not sure which 'convertToRVA:' this is? */
                                   withObject:cellContent];
      }
    }
...
- (void)tableView:(NSTableView *)aTableView setObjectValue:(id)anObject forTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex
{
  ...
  // option1: plain hex value
  {
    ...
    else
    {
      ...
      for (NSUInteger s = 0U; s < [cellContent length]; s += 2U) {
        buf[0] = orgstr[s];
        buf[1] = orgstr[s + 1U];
  ...
  } else { // option2: group of bytes:
    // find out file offset from the row index:
    fileOffset = (uint32_t)(selectedNode.dataRange.location + (16U * rowIndex) + (8U * (colIndex == DATA_HI_COLUMN)));
